...
### 2. 电工现场接线
记录插座编号、漏保（漏电保护器）状态、电表初值、线缆照片 URL、摊主签字（**未签字不允许送电**）；高风险摊位若被要求电工复检且未通过，不能接线。接线后摊位进入「营业中」。

### 3. 营业中临时加电（同箱负载重算）
增加烤炉/冰柜/灯牌/音响时：
- 实时重算 `同箱在营负载 + 本次负载（经系数折算）`，超过配电箱安全容量（额定 × 0.8）→ **自动驳回**并给出减载/错峰/迁备用线建议；
- 同时校验**预付电费余额**是否够本次加电费（2 元/100W），不足同样驳回，绝不允许透支；
- 通过则自动计收加电费、写入摊位统计，并实时体现在配电箱负载率上。
...
### 6. 责任链不断点的现场变动
- **夜间撤摊**：读电表终值 → 结算电费（1.5 元/度）→ 断电关闭，记录撤摊时间。
...
### 7. 闭市结算、档案与安全评级
闭市统一读表结算，每摊生成档案：用电量、电费、违规明细与扣分、临时加电次数/费用、跳闸事件与责任认定、整改照片。
评级（扣分为主，事故次数与高风险加权）：
...
1. `vendor02` 登录 → ② 给 A02「临时加电」选 2000W 烤炉 → 看到同箱重算**超载驳回**；换 500W 冰柜 → 允许并扣加电费。
...
| `POST /api/vendor/temp-power` | 摊主/管理员 | 临时加电（同箱重算，自动允许/驳回/计费） |
