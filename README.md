...
摊主临时增加烤炉/冰柜/灯牌/音响时提交审批，**入参先做硬校验：额定功率与数量必须为正整数（功率 1–20000W、数量 1–50），为 0、负数或越界直接返回 400，不生成申请记录、不触碰余额与累计费用**。合法请求依次检查：
...
**审批拒绝**（容量/冷柜/ETA/欠费/风控）：记录中费用恒为 0、`charged=false`，页面明确显示「未收费 0元」，返回业务拒绝并给出可执行的**替代建议**——降功率（反推可申报额定功率上限）/换低功率设备、错峰（22:30 邻近冷柜收摊后）、迁移到有余量的备用线路箱（自动计算剩余容量）、充值后重试等；拒绝不产生任何费用。
